...
本项目的亮点不只是“把后台管理做完整”，而是把 AI 能力放进真实业务流里：用户可以通过对话提出问题，系统结合会员资料、课程、教练、商品、会员卡、知识库等工具完成检索、推理和回答，并具有基于自然语言对话，实现AI自主决策与编排健身房约课、购卡、查数据等业务操作，提升用户全链路服务体验，释放前台及教练的运营人力成本。

## 使用界面

**建议保留图片和视频占位。** 这个项目有较多业务端和 AI 交互流程，纯文字很难让读者快速感知完成度。推荐至少放 3 张截图和 1 个短视频：

| 素材 | 建议位置 | 内容描述 |
|------|----------|----------|
| 项目总览图 | README 顶部或项目简介后 | 展示首页、会员端、教练端、管理端的组合截图 |
| AI 助手工具调度截图 | AI 能力章节 | 展示用户提问后，Agent 调用课程/教练/RAG/会员卡等工具的过程或结果 |
| 管理端数据看板截图 | 核心功能章节 | 展示运营指标、图表、数据分析报告 |
| 演示视频 | 快速体验章节前 | 1-3 分钟录屏，串起登录、预约、AI 问答、支付/订单、后台管理 |


<!--
![项目总览：公共首页、会员端、教练端、管理端四屏组合](docs/assets/readme/project-overview.png)
-->
### 首页
- 所有角色共用入口：
